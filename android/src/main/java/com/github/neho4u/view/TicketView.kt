...
//        setContentView(R.layout.a_ticket_view)
    ...
    private fun inflateTicket(ticket: Ticket) {
        ...
        if (ticket.lastUpdated != null) {
            ...
            noteBinding.tvTicketViewLastUpdated.text = "Updated ${ticket.lastUpdated}"
        }

        if (ticket.priority != null) {
            noteBinding.tvSeverity.visibility = View.VISIBLE
            noteBinding.tvSeverity.text = "Priority: ${ticket.priority}"
        }
        ...
        if (ticket.detail != null) {
            ...
            noteBinding.tvTicketViewDetails.text = Html.fromHtml(ticket.detail, FROM_HTML_MODE_LEGACY)
        }
        ...
        if (ticket.notes != null) {
            ...
            for (i in ticket.notes.indices) {
                ...
                val binding = NoteDetailBinding.bind(noteView)

                if (note.isSolution != null && note.isSolution) {
                    binding.tvNoteDetailResolution.visibility = View.VISIBLE
                } else {
                    binding.tvNoteDetailResolution.visibility = View.GONE
                }
                binding.tvNoteDetailPrettyLastUpdated.text = note.prettyUpdatedString
                binding.tvNoteDetailDetail.text = note.mobileNoteText
    ...
    override fun ticketError(error: String) {
        this.runOnUiThread {
            ...
            val snackbar: Snackbar = Snackbar.make(noteBinding.ticketViewParent, error, Snackbar.LENGTH_INDEFINITE)
